...
export default function FreelancesDemandesPage() {
  ...
  const fetchDemandes = async () => {
    ...
    setError(null)
    try {
      const data = await adminService.listFreelances('EN_ATTENTE')
      setDemandes(data)
    } catch (err) {
  ...
  const handleValider = async (chauffeurId) => {
    try {
      await adminService.validerFreelance(chauffeurId)
      setDemandes(prev => prev.filter(d => d.chauffeurId !== chauffeurId))
    } catch (err) {
  ...
  const handleRefuser = async () => {
    ...
    try {
      await adminService.refuserFreelance(selectedDemande.chauffeurId, motifRefus.trim())
      setDemandes(prev => prev.filter(d => d.chauffeurId !== selectedDemande.chauffeurId))
    ...
    <div className="space-y-6">
      ...
      {demandes.length === 0 ? (
        <div className="rounded-xl border p-12 text-center" style={{ borderColor: '#ECECEC', backgroundColor: '#FFFFFF' }}>
          <span className="material-symbols-outlined text-[48px]" style={{ color: '#ECECEC' }}>hail</span>
          <p className="font-body text-body-md mt-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#8A8A92' }}>
            Aucune demande de chauffeur freelance en attente
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {demandes.map((d) => (
            <div key={d.chauffeurId} className="rounded-xl border p-5 transition-all hover:shadow-sm" style={{ borderColor: '#ECECEC', backgroundColor: '#FFFFFF' }}>
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-start gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full text-[13px] font-bold" style={{ backgroundColor: '#F7F7F8', color: '#8A8A92' }}>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-4 text-[13px]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#8A8A92' }}>
                      {d.permisCategorie && (
                        <span className="flex items-center gap-1">
                          ...
                          Permis {d.permisCategorie}
                        </span>
                      )}
                      {d.experienceAnnees != null && (
                        <span className="flex items-center gap-1">
                          <span className="material-symbols-outlined text-[16px]">schedule</span>
                          {d.experienceAnnees} ans d'expérience
                        </span>
                      )}
                      {d.marqueModele && (
                        <span className="flex items-center gap-1">
                  ...
                  <div className="flex flex-col items-end gap-3 md:flex-row md:items-center">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleValider(d.chauffeurId)}
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2 border-t pt-4" style={{ borderColor: '#ECECEC' }}>
                  ...
                  {d.hasPermisScan ? (
                    <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-[11px] font-medium" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", backgroundColor: '#E8F5E9', color: '#1A1A1E' }}>
                      <span className="material-symbols-outlined text-[12px]">check_circle</span>
                      Permis de conduire
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-[11px] font-medium" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", backgroundColor: '#FDE8E6', color: '#E8433D' }}>
                      <span className="material-symbols-outlined text-[12px]">error</span>
                      Permis manquant
                    </span>
                  )}
                  <button
                    onClick={() => onNavigate('freelance_detail', { chauffeurId: d.chauffeurId, fromDemandes: true })}
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 rounded-t-2xl p-6 pb-10 shadow-xl" style={{ backgroundColor: '#FFFFFF' }}>
            ...
            <p className="font-body text-body-sm mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#8A8A92' }}>
              {selectedDemande?.nom || 'Ce chauffeur'} — Motif du refus obligatoire
            </p>
            <textarea
              value={motifRefus}
